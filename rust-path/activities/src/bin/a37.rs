...
#[derive(Debug, Error)]
enum RgbError {
    #[error("hex color must be has a (#)")]
    MisingHash,
    #[error("hex color cant convert {0}")]
    ParseError(#[from] std::num::ParseIntError),
    #[error("hex color provided is not a valid")]
    LengthError,
}
...
impl TryFrom<&str> for Rgb {
    ...
    fn try_from(value: &str) -> Result<Self, Self::Error> {
        if !value.starts_with("#") {
            return Err(RgbError::MisingHash);
        }
        if value.len() != 7 {
        ...
        let (r, g, b) = (
            ...
            u8::from_str_radix(&value[5..=6], 16)?,
        );
        Ok(Self(r, g, b))
    }
}
