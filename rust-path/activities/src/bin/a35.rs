...
fn print_tile(tile: Tile) {
    use Tile::*;
    match tile {
        Brick(brick @ BrickStyle::Dungeon | brick @ BrickStyle::Red) => {
            println!("the brick color is {:?}", brick)
        }
        Brick(other) => println!("the other brick is {:?}", other),
        Dirt | Grass | Sand => println!("ground tile"),
        Treasure(TreasureChest { amount, .. }) if amount >= 100 => println!("Lot of gold"),
        Water(pressure) if pressure.0 < 100 => println!("Water presure level {}", pressure.0),
        Water(pressure) if pressure.0 >= 100 => println!("High Water presure level {}", pressure.0),
        Wood => println!("Wood"),
        _ => (),
    }
}
